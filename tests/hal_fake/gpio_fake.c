...
static void edge_sample_snapshot(void) {
    if (!s_edge.configured)
        return;
    if (s_edge.count >= HAL_FAKE_GPIO_EDGE_LOG_MAX)
        return;
    hal_fake_gpio_edge_sample_t* e = &s_edge.log[s_edge.count++];
    for (int i = 0; i < 4; i++) {
        hal_gpio_pin_t p = s_edge.watch[i];
        e->watch[i]      = (p < HAL_FAKE_GPIO_MAX_PINS) ? hal_fake_gpio_states[p].level : false;
    }
}
...
void hal_gpio_put(hal_gpio_pin_t pin, bool value) {
    ...
    if (s_edge.configured && pin == s_edge.trigger && !prev && value) {
...
void hal_fake_gpio_input_script_load(hal_gpio_pin_t pin, const bool* bits, size_t bit_count) {
    if (pin >= HAL_FAKE_GPIO_MAX_PINS || bits == NULL)
        return;
...
void hal_fake_gpio_edge_sampler_configure(hal_gpio_pin_t trigger, hal_gpio_pin_t w0,
    ...
    s_edge.watch[2]   = w2;
    s_edge.watch[3]   = w3;
    s_edge.last_trigger_level =
        (trigger < HAL_FAKE_GPIO_MAX_PINS) ? hal_fake_gpio_states[trigger].level : false;
    s_edge.count = 0;
}
