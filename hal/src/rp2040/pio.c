...
static inline PIO as_pio(hal_pio_inst_t* p) {
    return (PIO)p;
}

hal_pio_inst_t* hal_pio_instance(uint8_t which) {
    if (which == 0)
        return (hal_pio_inst_t*)pio0;
    if (which == 1)
        return (hal_pio_inst_t*)pio1;
    return NULL;
}
...
bool hal_pio_can_add_program(hal_pio_inst_t* pio, const hal_pio_program_t* p) {
    if (!pio || !p)
        return false;
    pio_program_t prog = {
        .instructions = p->instructions, .length = (uint8_t)p->length, .origin = (int8_t)p->origin};
...
bool hal_pio_claim_sm(hal_pio_inst_t* pio, uint32_t sm) {
    if (!pio || sm > 3)
        return false;
    if (pio_sm_is_claimed(as_pio(pio), (uint)sm))
        return false;
...
void hal_pio_sm_configure(hal_pio_inst_t* pio, uint32_t sm, uint32_t offset,
    ...
    if (cfg->sideset_pin_count) {
        ...
        uint sideset_bits = cfg->sideset_pin_count + (cfg->sideset_optional ? 1u : 0u);
        sm_config_set_sideset(&c, sideset_bits, cfg->sideset_optional, cfg->sideset_pindirs);
    ...
    if (cfg->wrap_end > cfg->wrap_target) {
        ...
        sm_config_set_wrap(&c, (uint)offset + cfg->wrap_target, (uint)offset + cfg->wrap_end);
    }
...
bool hal_pio_sm_try_put(hal_pio_inst_t* pio, uint32_t sm, uint32_t word) {
    if (!pio || sm > 3)
        return false;
    if (pio_sm_is_tx_fifo_full(as_pio(pio), (uint)sm))
        return false;
...
bool hal_pio_sm_try_get(hal_pio_inst_t* pio, uint32_t sm, uint32_t* out) {
    if (!pio || sm > 3 || !out)
        return false;
    if (pio_sm_is_rx_fifo_empty(as_pio(pio), (uint)sm))
        return false;
...
void hal_pio_set_consecutive_pindirs(hal_pio_inst_t* pio, uint32_t sm, uint32_t base,
    ...
    if (!pio || sm > 3)
        return;
    pio_sm_set_consecutive_pindirs(as_pio(pio), (uint)sm, (uint)base, (uint)count, is_out);
}
